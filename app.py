...
import os
import pandas as pd
from utils.visualization import (sales_by_category, sales_trend, region_sales, top_products, sales_by_segment, profit_by_category, top_customers, correlation_heatmap)
...
def clean_data():
    ...
    if "encode_categorical" in request.form:
        ...
        # Encode only non-date text columns
...
def train_model():
    ...
    if problem == "Classification":
        ...
        # Split again with encoded target
        X_train, X_test, y_train, y_test = train_test_split(
            X,
            y,
            test_size=0.2,
...
# Remove target if it exists in uploaded file
...
def run_forecast():
    ...
    chart, forecast = generate_forecast(
